...
export const HomePage = () => {
  const [searchParams, setSearchParamss] = useState(`?_page=1&_per_page=${DEFAULT_PER_PAGE}`);
  ...
  const onSortSelectChangeHandler = (e) => {
    ...
    setSearchParamss(`?_page=1&_per_page=${DEFAULT_PER_PAGE}&${e.target.value}`);
  };

  const paginationHandler = (e) => {
    if (e.target.tagName === "BUTTON") {
      setSearchParamss(`?_page=${e.target.textContent}&_per_page=${DEFAULT_PER_PAGE}&${sortSelectValue}`);
    ...
    <>
      <div className={cls.controlsContainer} ref={controlsContainerRef}>
        ...
        <select value={sortSelectValue} onChange={onSortSelectChangeHandler} className={cls.select}>
          ...
          <option value="_sort=completed">complited ASK</option>
