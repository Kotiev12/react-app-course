...
export const QuestionPage = () => {
  ...
  const [updateCard, isCardUpdaiting] = useFetch(async (isChecked) => {
    const response = await fetch(`${API_URL}/react/${id}`, {
      method: "PATCH",
        ...
        <div className={cls.container}>
          ...
          <label htmlFor={checkboxId} className={cls.cardCheckBox}>
            <input
              ...
              disabled={isCardUpdaiting}
            />
            <span>mark question as completed</span>

            {isCardUpdaiting && <SmallLoader />}
          </label>

          <Button onClick={() => navigate(`/editquestion/${card.id}`)} isDisabled={isCardUpdaiting}>
            Edit Question
          </Button>
          <Button onClick={() => navigate("/")} isDisabled={isCardUpdaiting}>
            Back
          </Button>
        </div>
      )}
